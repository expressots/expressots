 ...
 * build-esm.js
 *
 * Atomic ESM build for a provider package whose source `package.json` is
 * intentionally CJS-default (no `"type": "module"`).
 *
 * Why this dance is needed:
 *   - We want `module: NodeNext` in tsconfig.esm.json so TypeScript validates
 *     the `.js` extension on every relative import and emits valid ESM.
 *   - With `module: NodeNext`, TypeScript decides whether to emit CJS or ESM
 *     by reading the SOURCE file's nearest `package.json` `"type"` field.
 *   - We can't permanently set `"type": "module"` in the source `package.json`
 *     because that breaks the CJS build for consumers of `lib/cjs/*.js`.
 *
 * So we:
