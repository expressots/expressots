...
const PROVIDERS: CloudProvider[] = ["aws", "gcp", "azure", "railway", "render", "fly", "digitalocean", "heroku"];
...
export async function compareCosts(options: CostOptions): Promise<void> {
	...
	for (const est of estimates) {
		...
		const diffStr = costDiff > 0 ? chalk.gray(`+$${costDiff.toFixed(2)}`) : chalk.green("Best");
		
		const providerColor = rank === 1 ? chalk.green : rank <= 3 ? chalk.yellow : chalk.white;
	...
	console.log(`  Cheapest: ${chalk.green(estimates[0].provider)} at $${estimates[0].monthlyCost.toFixed(2)}/month`);
	...
	if (estimates.length > 1) {
		...
		if (savingsVsHeroku && savingsVsHeroku.monthlyCost > cheapest) {
			...
			console.log(`  Save ${chalk.yellow(`$${savings.toFixed(2)}/month`)} vs Heroku by switching to ${estimates[0].provider}`);
		}
	}
	...
	if (cheapestPaaS && cheapestCloud) {
		if (cheapestPaaS.monthlyCost < cheapestCloud.monthlyCost) {
			console.log(`  For simplicity: Use ${chalk.cyan(cheapestPaaS.provider)} (PaaS)`);
		} else {
			console.log(`  For control: Use ${chalk.cyan(cheapestCloud.provider)} (IaaS) - ${chalk.gray("requires more setup")}`);
		}
	}
...
export async function optimizeCosts(options: CostOptions): Promise<void> {
	...
	const analysis = await analyzeProject();
	const recommendations: { priority: string; title: string; savings: string; description: string }[] = [];
...
export async function showPricing(options: CostOptions): Promise<void> {
	...
	if (dynamicPricing) {
		console.log(chalk.gray(`  Source: ${source || "dynamic"} (v${dynamicPricing.version})`));
		console.log();
	}
	...
	if (options.provider) {
		...
		// Show all providers
		for (const provider of PROVIDERS) {
			const dynamicProvider = dynamicPricing?.providers[provider as keyof typeof dynamicPricing.providers];
	...
	console.log(chalk.gray("Note: Prices are estimates and may vary by region. Check provider websites for current pricing."));
	if (!dynamicPricing) {
		console.log(chalk.yellow("  Using embedded pricing data. Run 'expressots costs update' to fetch latest prices."));
	}
	console.log();
}
 ...
 */
async function getDynamicOrFallbackPricing(provider: CloudProvider): Promise<ProviderPricing> {
	const pricingManager = getPricingManager();
	const dynamicPricing = await pricingManager.fetchPricing();
...
function calculateEstimate(
	provider: CloudProvider,
	resources: { instances: number; cpu: number; memory: number; storage: number; bandwidth: number; hours: number },
	options: CostOptions
): CostEstimate {
	...
	const bandwidthCost = Math.max(0, resources.bandwidth - pricing.freeBandwidth) * pricing.bandwidthPerGb;
	
	// Provider-specific calculations
	if (pricing.model === "per-hour") {
		computeCost = resources.instances * resources.cpu * pricing.cpuPerHour * resources.hours;
		memoryCost = resources.instances * resources.memory * pricing.memoryPerGbHour * resources.hours;
		...
		memoryCost = resources.memory * pricing.memoryPerGbHour * resources.hours;
	}
...
function printEstimate(estimate: CostEstimate, options: CostOptions): void {
	...
	console.log("-".repeat(40));
	console.log(chalk.bold.cyan(`  Total:     $${estimate.monthlyCost.toFixed(2)}/month`));
	console.log();
}
 ...
 */
function printProviderPricing(provider: string, pricing: ProviderPricing): void {
	console.log(chalk.bold(`${provider.toUpperCase()} - ${pricing.serviceName}`));
