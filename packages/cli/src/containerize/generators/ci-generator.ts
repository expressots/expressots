...
export async function generateCIConfig(
	...
	printSection(`📝 Generating CI/CD configuration for ${platform}`);
...
async function generateAllPlatforms(
	...
	printBullet(chalk.green(`✓ Generated CI/CD configs for all platforms`));
}
 ...
 */
function printSinglePipelinePlatformWarning(platform: CIPlatform): void {
	stdout.write("\n");
	printWarning(
		`${platform} only supports a single canonical pipeline config file.`,
		"cicd",
	);
	printBullet(
		chalk.gray(
			"The CD-focused output below will overwrite anything previously",
		),
	);
	printBullet(
		chalk.gray(
			"generated by `expressots cicd init`. For non-GitHub platforms",
		),
	);
	printBullet(
		chalk.gray(
			"prefer `cicd init` (which already includes Docker build/push/deploy).",
		),
	);
	stdout.write("\n");
}
...
async function generateGitHubActions(
	...
	fs.writeFileSync(path.join(githubDir, "cd-docker.yml"), workflow, "utf-8");
	printBullet(chalk.green(`✓ Created .github/workflows/cd-docker.yml`));
}
...
async function generateGitLabCI(
	...
	printBullet(chalk.green(`✓ Created .gitlab-ci.yml`));
}
...
async function generateCircleCI(
	...
	printBullet(chalk.green(`✓ Created .circleci/config.yml`));
}
...
async function generateJenkinsfile(
	...
	printBullet(chalk.green(`✓ Created Jenkinsfile`));
}
...
async function generateBitbucketPipelines(
	...
	printBullet(chalk.green(`✓ Created bitbucket-pipelines.yml`));
}
...
async function generateAzureDevOps(
	...
	printBullet(chalk.green(`✓ Created azure-pipelines.yml`));
}
...
function getInstallCommand(packageManager: string): string {
	return getCiInstallCommand(packageManager);
}
