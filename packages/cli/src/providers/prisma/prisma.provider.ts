	...
	if (answer.confirm) {
		...
		if (packageManager) {
			...
			if (answer.installDriver) {
				...
				console.log(
					`Installing the latest recommended database driver for ${
						answer.databaseName
					}: ${drivers[answer.databaseName]} ...`,
				);
				await execProcess({
					commandArg: packageManager,
		...
		answer.databaseName = choices.find(
			(choice) => choice.name === answer.databaseName,
		)?.value;
		...
		const newFileName = path.join(
			process.cwd(),
			`/prisma/${answer.schemaName}.prisma`,
		);
		...
		const schemaPath = path.join(
			process.cwd(),
			`${answer.schemaPath}/prisma/${answer.schemaName}.prisma`,
		);
		if (!fs.existsSync(schemaPath)) {
			fs.mkdirSync(
				path.join(process.cwd(), `${answer.schemaPath}/prisma`),
				{ recursive: true },
			);
		}
		...
		if (answer.baseRepository) {
			...
			const baseRepositoryInterfaceTplPath = path.join(
				__dirname,
				"./templates/base-repository.interface.tpl",
			);
			const baseRepositoryTplPath = path.join(
				__dirname,
				"./templates/base-repository.tpl",
			);

			const baseRepositoryInterfaceTemplate = fs.readFileSync(
				baseRepositoryInterfaceTplPath,
				"utf8",
			);
			const baseRepositoryTemplate = fs.readFileSync(
				baseRepositoryTplPath,
				"utf8",
			);
			fs.writeFileSync(
		...
		await addProviderConfigInExpressotsConfig(
			answer.schemaName,
			`${answer.schemaPath}/prisma`,
		);
...
async function addProviderConfigInExpressotsConfig(
	schemaName: string,
	schemaPath: string,
): Promise<void> {
	const absDirPath = path.resolve(process.cwd());
