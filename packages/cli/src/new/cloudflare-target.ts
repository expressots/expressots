...
/** Where the generated iconv-lite stub lives, relative to the project root. */
const ICONV_SHIM_PATH = "./src/shims/iconv-lite.cjs";
...
export default defineConfig({
    // The pool does not read wrangler.toml's \`alias\`, so the iconv-lite shim
    // has to be declared here as well. Without it the suite would run against
    // the real iconv-lite while the deployed Worker runs against the stub, and
    // a test suite that does not match production is the exact problem running
    // tests on workerd is meant to solve.
    resolve: {
        alias: {
            "iconv-lite": fileURLToPath(
                new URL("${ICONV_SHIM_PATH}", import.meta.url),
            ),
        },
    },
    plugins: [
...
/**
 * Stub for \`iconv-lite\`, aliased in for the Worker build.
 *
 * \`express\` imports \`body-parser\`, which imports \`iconv-lite\` to decode
 * request bodies in non-UTF-8 charsets. Its encoding tables are ~485 KiB
 * (the CJK codecs alone are 424 KiB), and a Workers build cannot perform the
 * dynamic \`require\` iconv-lite uses on Node, so every table is bundled.
 *
 * None of it is reachable on this target: the adapter parses request bodies
 * itself and Express's body middleware is disabled, so body-parser's read
 * path never executes. Aliasing it away is worth ~195 KiB gzipped.
 *
 * CommonJS on purpose — \`raw-body\` and \`body-parser\` reach it through
 * \`require()\`, and an ESM shim breaks their interop under Vitest.
 */
const CLOUDFLARE_ICONV_SHIM = `/**
...
- \`src/shims/iconv-lite.cjs\` is a build shim aliased in by \`wrangler.toml\`
  and \`vitest.config.mts\`. Do not edit or import it, and do not delete it
  without removing both alias entries.
...
src/shims/iconv-lite.cjs   Build shim — see Bundle size below
...
\`src/shims/iconv-lite.cjs\` replaces the real \`iconv-lite\` in the Worker
build. Express pulls it in through \`body-parser\` to decode request bodies in
non-UTF-8 charsets — about 485 KiB of encoding tables, all of which get
bundled because a Workers build cannot do the dynamic \`require\` iconv-lite
uses on Node.

That code never runs here: the adapter parses request bodies itself and
Express's body middleware is disabled on this target. Aliasing it away saves
roughly 195 KiB gzipped.

The alias is declared twice on purpose — in \`wrangler.toml\` for the deployed
build and in \`vitest.config.mts\` for the test build, which does not read
\`wrangler.toml\`. If you delete the shim, remove both.
...
export function applyCloudflareTarget({
	...
	const iconvShimPath = path.join(targetDir, ...ICONV_SHIM_PATH.split("/"));
	fs.mkdirSync(path.dirname(iconvShimPath), { recursive: true });
	fs.writeFileSync(iconvShimPath, CLOUDFLARE_ICONV_SHIM, "utf8");
	fs.writeFileSync(
		wranglerPath,
		[
			...
			`alias = { "iconv-lite" = "${ICONV_SHIM_PATH}" }`,
			"",
		].join("\n"),
		"utf8",
	);
