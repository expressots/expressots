...
function generateWorkflowContentEmbedded(options: GeneratorOptions): string {
	...
	// GitHub-hosted runners ship Node; Bun is provisioned via
	// `oven-sh/setup-bun`. Other PMs use `actions/setup-node` with its
	// built-in dependency cache (which does not support Bun).
	const setupSteps =
		packageManager === "bun"
			? `      - name: Setup Bun
