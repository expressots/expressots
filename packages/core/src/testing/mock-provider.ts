...
function detectTestFramework(): TestFramework {
  ...
  if (
    typeof globalWithVi.vi !== "undefined" &&
...
function createMockFunction<
  ...
  R = unknown,
>(framework: TestFramework): MockFunction<A, R> {
...
export function mockProvider<T>(
  ...
  const effectiveFramework =
    framework !== "unknown" ? framework : mockFramework;
  ...
  for (let i = 0; i < dependencies.length; i++) {
    ...
    if (userMock) {
      ...
      const autoMocked = createAutoMock(
        dep as new (...args: Array<unknown>) => unknown,
        effectiveFramework,
      );
    ...
    for (const [name, mockObj] of Object.entries(mockObjects)) {
      if (mockObj && typeof mockObj === "object") {
        for (const [fnName, fn] of Object.entries(
          mockObj as Record<string, unknown>,
        )) {
...
export function createMockObject<T>(
  ...
  return createAutoMock(classRef, detectTestFramework()) as MockDependencies<
    typeof classRef
  >;
}
...
export function spyOn<T extends object, K extends keyof T>(
  ...
  // Add restore method
  const mockFnWithRestore = mockFn as MockFunction & {
    mockRestore: () => void;
  };
  mockFnWithRestore.mockRestore = (): void => {
    (obj as Record<string | symbol, unknown>)[method as string | symbol] =
      original;
  };
