...
function _getActiveBindings(
  ...
  let activeBindings: Array<interfaces.Binding<unknown>> = [];
...
): Array<interfaces.Binding<T>> {
  let bindings: Array<interfaces.Binding<T>> = [];
  const bindingDictionary = getBindingDictionary(container);

  if (bindingDictionary.hasKey(serviceIdentifier)) {
    bindings = bindingDictionary.get(serviceIdentifier) as Array<
