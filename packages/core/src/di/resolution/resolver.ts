...
const _resolveRequest =
  ...
  ): undefined | T | Promise<T> | Array<T | Promise<T>> => {
...
): Iterator<interfaces.BindingActivation<T>> => {
...
const _getContainersIterator = (
  ...
  const containersStack: Array<interfaces.Container> = [container];
...
function resolve<T>(
  context: interfaces.Context,
): T | Promise<T> | Array<T | Promise<T>> {
  const _f = _resolveRequest<T>(
    context.plan.rootRequest.requestScope as interfaces.RequestScope,
  );
  return _f(context.plan.rootRequest) as T | Promise<T> | Array<T | Promise<T>>;
}
