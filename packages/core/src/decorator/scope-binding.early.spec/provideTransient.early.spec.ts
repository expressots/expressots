...
describe("provideTransient() provideTransient method", () => {
  describe("Happy Path", () => {
    it("should return a decorator function when called with a valid identifier", () => {
      ...
      // Act
      const result = provideTransient(identifier);
    ...
    it("should store metadata with Transient scope when decorator is applied", () => {
      // Arrange
      const identifier = "ServiceIdentifier";
      class TestClass {}

      // Act
      const decorator = provideTransient(identifier);
    ...
    it("should use custom source when provided", () => {
      ...
      // Act
      const decorator = provideTransient(identifier, source);
      decorator(TestClass);
