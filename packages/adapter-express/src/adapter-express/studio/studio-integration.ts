...
async function isStudioAgentInstalled(): Promise<boolean> {
  ...
  try {
    ...
    if (debug) console.log('[Studio] Cannot resolve studio-agent:', error instanceof Error ? error.message : error);
...
export async function initializeStudio(
  ...
  try {
    ...
    const StudioAgent =
      studioAgentModuleAny.StudioAgent ||
      studioAgentModuleAny.default?.StudioAgent;
