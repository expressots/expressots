...
export class ServiceDiscovery {
    ...
    registerService(service: ServiceInstance): void {
        ...
        if (this.config.debug) {
            console.log(
                `[ServiceDiscovery] Registered ${service.name} (${service.id})`
            );
        }
    }
    ...
    updateHealth(
        ...
        if (this.config.debug) {
            console.log(
                `[ServiceDiscovery] Updated ${serviceName} (${instanceId}) health to ${health}`
            );
        }
    ...
    getStats(): Record<
        string,
        { total: number; healthy: number; unhealthy: number }
    > {
        const stats: Record<
            string,
            { total: number; healthy: number; unhealthy: number }
        ...
        for (const [name, instances] of this.services) {
            stats[name] = {
                ...
                unhealthy: instances.filter((i) => i.health === "unhealthy")
                    .length,
            };
        }
    ...
    private async refreshFromConsul(): Promise<void> {
        ...
        try {
            ...
            const servicesResponse = await fetch(
                `${this.config.endpoint}/v1/catalog/services`
            );
            ...
            if (this.config.debug) {
                console.log(
                    `[ServiceDiscovery] Refreshed ${this.services.size} services from Consul`
                );
            }
        } catch (error) {
     ...
     */
    private async getConsulInstances(
        serviceName: string
    ): Promise<Array<ServiceInstance>> {
        const response = await fetch(
            `${this.config.endpoint}/v1/health/service/${serviceName}?passing=true`
        );
        const data = (await response.json()) as Array<{
            Service: { ID: string; Service: string; Address: string; Port: number; Meta?: Record<string, unknown> };
            Node: { Address: string };
        }>;
    ...
    private async refreshFromEtcd(): Promise<void> {
        ...
        console.log(
            "[ServiceDiscovery] etcd integration requires etcd3 client library"
        );
    }
}
