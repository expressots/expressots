 ...
 *
 * `TRequest` is the request type route handlers receive. It defaults to
 * `express.Request`; serverless adapters that decorate the request (for
 * example `CloudflareRequest<Env>`) can be named here so handlers are typed
 * without casts.
 * @public API
 */
...
export function micro<TRequest extends express.Request = express.Request>(
  ...
  const wrapHandler = (handler: RouteHandler<TRequest>): express.RequestHandler => {
    return async (req, res, next) => {
      try {
        // The adapter for the target runtime is what decorates the request
        // (e.g. cloudflareAdapter adds `cloudflare` and `services`); the
        // route only names the shape it expects.
