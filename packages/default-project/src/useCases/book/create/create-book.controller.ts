...
class CreateBookController extends BaseController{
    ...
    @httpPost("/")
    execute(@requestBody() data: ICreateBookDTO, @response() res: any): ICreateBookResponseDTO {
        
        return this.callUseCase(this.createBookUseCase.execute(data), res, StatusCode.Created);
    }
}
